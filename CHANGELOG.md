...
- extract hardcoded Workday API pagination and safety limits to configurable constants `WORKDAY_PAGE_LIMIT` and `WORKDAY_MAX_JOBS_PER_COMPANY` ([#43](https://github.com/ambicuity/New-Grad-Jobs/issues/43))
- enhance `safe_str` in `get_job_key` to robustly handle `numpy.floating` NaNs and Infs when deduping jobs originating from JobSpy/pandas
