...
| `corpus-index.json` | Every unique posting the run saw (about 54k), titles only, as compact rows `[company, title, location, source, posted_at, category, tier, url]` where tier is 0 curated / 1 near miss / 2 out. The data foundation for an opt-in "explore everything" mode with viewer-defined signals (`site/src/lib/signals.js` is a parity-tested port of the scraper's matcher). ~1.2 MB as a Brotli sibling. Never loaded by default, no pages, not in the sitemap. |
