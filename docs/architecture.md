...
- **GitHub Pages**: The `docs/` directory hosts a React "terminal" UI (`docs/terminal/*.jsx`, transpiled in-browser via Babel Standalone — no build step) that fetches the raw `jobs.json` from the `main` branch to render a fast, filterable UI for end-users, deployed automatically by GitHub.
