...
2. **Pre-commit (`Code Hygiene (Pre-commit)`)**:
   * **Why it matters**: It automatically catches messy formatting, merge markers, oversized files, private keys, and secret leaks so you don't have to leave nitpicky review comments. If it fails, ask the contributor to run `pre-commit run --all-files` locally before you merge.
3. **CodeQL (`CodeQL Security Scan`)**:
   * **Why it matters**: Static analysis for Python code vulnerabilities. Runs per-PR and on a weekly cron (`0 8 * * 1`). GitHub will email you on critical findings — you only need to act if you get that alert or the check blocks a PR.
4. **Scraper publisher (`Update New Grad Jobs`)**:
   * **Why it matters**: This is the scheduled scraper (`*/5 * * * *`). It pulls `main` with `--autostash`, writes generated docs artifacts, and retries rejected pushes up to three times. On a non-`main` ref triggered manually via `workflow_dispatch`, it uploads prediction artifacts instead of committing — handy for verifying changes before they land on `main`.
5. **Dependabot PRs**:
   * **Why it matters**: `.github/dependabot.yml` groups weekly GitHub Actions and pip updates into a small number of PRs. There is no auto-merge workflow — review and merge them yourself once the gatekeeper checks above pass.

**TL;DR:** If `CI — Lint & Validate`, `Code Hygiene (Pre-commit)`, and `CodeQL Security Scan` are all green, the code is safe to merge. Intervene only if one of those turns red, or when scheduled CodeQL emails you about a new alert.
