#!/usr/bin/env python3
"""Behavioral tests for the commit/push step of .github/workflows/update-jobs.yml.
...
def _commit_step_script() -> str:
    ...
    steps = workflow["jobs"]["update-jobs"]["steps"]
...
def _write_site(repo: pathlib.Path, jobs_payload: dict) -> None:
    docs = repo / "docs"
    (docs / "descriptions").mkdir(parents=True, exist_ok=True)
    (docs / "jobs.json").write_text(json.dumps(jobs_payload, indent=2) + "\n")
    (docs / "jobs-index.json").write_text(json.dumps({"n": len(jobs_payload["jobs"])}) + "\n")
    (docs / "market-history.json").write_text("{}\n")
    (docs / "health.json").write_text("{}\n")
    (docs / "feed.xml").write_text("<rss/>\n")
    (docs / "descriptions" / "a.txt").write_text("a\n")
    (repo / "README.md").write_text("# Jobs\n")
...
def test_pushes_when_upstream_advanced_without_conflict(repos):
    ...
    assert json.loads(_origin_file(origin, env, "docs/jobs.json")) == {"jobs": ["fresh"]}


def test_conflicting_upstream_change_fails_without_pushing_markers(repos):
    env, origin, runner, upstream = repos
    (upstream / "docs" / "jobs.json").write_text(json.dumps({"jobs": ["upstream"]}, indent=2) + "\n")
    _git(upstream, env, "commit", "-am", "manual jobs.json edit")
    _git(upstream, env, "push", "origin", "main")
    ...
    assert "<<<<<<<" not in _origin_file(origin, env, "docs/jobs.json")
...
def test_invalid_generated_json_is_never_pushed(repos):
    ...
    before = _git(origin, env, "rev-parse", "main").stdout
    _write_site(runner, {"jobs": ["fresh"]})
    (runner / "docs" / "jobs-index.json").write_text('{"truncated": \n')
...
def test_conflict_markers_are_never_pushed(repos):
    ...
    before = _git(origin, env, "rev-parse", "main").stdout
    _write_site(runner, {"jobs": ["fresh"]})
    (runner / "docs" / "feed.xml").write_text("<<<<<<< Updated upstream\n<rss/>\n=======\n>>>>>>> Stash\n")
